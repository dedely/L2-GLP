	...
	public Unit create(String type, Coordinates spawnPosition, Faction faction) throws IllegalArgumentException {
		if (type == Constants.ARTILLERY) {
			if (faction.getName() == Constants.UNION) {
				...
				return new Artillery("Elephant", faction,600, 400, 20, Constants.LIGHT,
				...
				return new ArtilleryLikeInfantry("Mortar Team", faction,200, 100, 5, Constants.SUPER_LIGHT,
			...
			if (faction.getName() == Constants.UNION) {
				return new TransportHelicopter("HTH", faction,800, 700, 15, Constants.LIGHT,
						"Heavy transport helicopter of the Union",
						spawnPosition, 50, 0, Constants.NO_MAIN_WEAPON, 0,
						false, 0, 15, 20);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new AttackHelicopter("NightJar", faction,900, 800, 30, Constants.LIGHT,
						"Attack helicopter of the federation",
						spawnPosition, 60, 40, Constants.LIGHT, 15, 500, true,
						120, 30, 8, Constants.HEAVY);

			} else if (faction.getName() == Constants.REPUBLIC) {
				return new AttackHelicopter("Wrath", faction,600, 400, 35, Constants.SUPER_LIGHT,
						"Helicopter-mounted rocket platform of the Republic",
						spawnPosition, 50, 0,
						Constants.NO_MAIN_WEAPON, 0, 450, false, 200, 10, 12, Constants.HEAVY);
			} else {
				throw new IllegalArgumentException("The faction " + faction.getName() + " doesn't exist !");
			}
		}
			...
			if (faction.getName() == Constants.UNION) {
				return new GroundUnit("Specialist", faction,100, 50, 10, Constants.SUPER_LIGHT,
						"The special force of the Union",
						spawnPosition, 10, 20, Constants.SUPER_LIGHT, 8, 100, true,
						1);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new GroundUnit("Ranger", faction,150, 50, 10, Constants.SUPER_LIGHT,
						"Foot soldier of the Federation",
						spawnPosition, 10, 30, Constants.SUPER_LIGHT, 12, 100, true,
						1);

			} else if (faction.getName() == Constants.REPUBLIC) {
				return new GroundUnit("Trooper", faction,75, 50, 5, Constants.SUPER_LIGHT,
			...
			if (faction.getName() == Constants.UNION) {
				return new GroundUnit("AT Soldier", faction,150, 50, 10, Constants.SUPER_LIGHT,
						"The anti-tank infantery support",
						spawnPosition, 8, 100, Constants.HEAVY, 70, 100, true, 1);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new GroundUnit("Mechanised Infantery", faction,200, 50, 10, Constants.LIGHT,
			...
			if (faction.getName() == Constants.UNION) {
				return new GroundUnitWithMountedWeapon("MBT", faction,500, 500, 20, Constants.HEAVY,
						"Main battle tank of the Union",
						spawnPosition, 20, 100, Constants.HEAVY, 100, 100, true, 12,
						false, 30, 20, true);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new GroundUnit("Lion", faction,600, 500, 30, Constants.HEAVY,
						"Heavy armored vehicle from the Federation",
						spawnPosition, 16, 120, Constants.SUPER_LIGHT, 120,
						100, true, 15);

			} else if (faction.getName() == Constants.REPUBLIC) {
				return new GroundUnit("Justice", faction,400, 400, 20, Constants.LIGHT,
			...
			if (faction.getName() == Constants.UNION) {
				return new LightTV("ASSV", faction,300, 300, 20, Constants.LIGHT,
						"Advanced support and scouting Vehicle of the Union",
						spawnPosition, 45, 45, Constants.LIGHT, 15, 80, true, 8, 5);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new LightTV("Gepard", faction,500, 400, 20, Constants.LIGHT,
						"Infantry armored transport from the Federation",
						spawnPosition, 35, 60, Constants.LIGHT, 30, 70, true, 12, 12);
			} else if (faction.getName() == Constants.REPUBLIC) {
				return new LightTV("Justice", faction,300, 250, 10, Constants.LIGHT, "Armored car of the republic",
			...
			if (faction.getName() == Constants.UNION) {
				return new Worker("MCM", faction,500, 400, 10, Constants.LIGHT,
						"Multi-porpose construction medium of the Union",
						spawnPosition, 20, 200, Constants.RESSOURCE, 10, 1, 14, false);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new Worker("Turtle", faction,600, 400, 10, Constants.LIGHT,
						"Heavy duty logistic vehicle from the federation",
						spawnPosition, 15, 300, Constants.RESSOURCE, 30, 1, 14, false);
			} else if (faction.getName() == Constants.REPUBLIC) {
				return new Worker("Worker", faction,100, 50, 5, Constants.SUPER_LIGHT,
						"Armored car of the republic",
						spawnPosition, 10, 75, Constants.RESSOURCE, 5, 1, 14, false);
			} else {
				throw new IllegalArgumentException("The faction " + faction.getName() + " doesn't exist !");
			}
		}
