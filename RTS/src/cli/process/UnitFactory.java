	...
	public Unit create(String type, Coordinates spawnPosition, Faction faction) throws IllegalArgumentException {
		if (type == Constants.ARTILLERY) {
			if (faction.getName() == Constants.UNION) {
				return new ArtilleryWithMountedWeapon("SPH", faction, 400, 20, Constants.LIGHT,
				...
				return new Artillery("Elephant", faction, 400, 20, Constants.LIGHT,
				...
				return new ArtilleryLikeInfantry("Mortar Team", faction, 100, 5, Constants.SUPER_LIGHT,
			...
			if (faction.getName() == Constants.UNION) {
				return new TransportHelicopter("HTH", faction, 700, 15, Constants.LIGHT,
						"Heavy transport helicopter of the Union",
						spawnPosition, 50, 0, Constants.NO_MAIN_WEAPON, 0,
						false, 0, 15, 20);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new AttackHelicopter("NightJar", faction, 800, 30, Constants.LIGHT,
						"Attack helicopter of the federation",
						spawnPosition, 60, 40, Constants.LIGHT, 15, 500, true,
						120, 30, 8, Constants.HEAVY);

			} else if (faction.getName() == Constants.REPUBLIC) {
				return new AttackHelicopter("Wrath", faction, 400, 35, Constants.SUPER_LIGHT,
			...
			if (faction.getName() == Constants.UNION) {
				return new GroundUnit("Specialist", faction, 50, 10, Constants.SUPER_LIGHT,
						"The special force of the Union",
						spawnPosition, 10, 20, Constants.SUPER_LIGHT, 8, 100, true,
						1);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new GroundUnit("Ranger", faction, 50, 10, Constants.SUPER_LIGHT,
						"Foot soldier of the Federation",
						spawnPosition, 10, 30, Constants.SUPER_LIGHT, 12, 100, true,
						1);

			} else if (faction.getName() == Constants.REPUBLIC) {
				return new GroundUnit("Trooper", faction, 50, 5, Constants.SUPER_LIGHT,
			...
			if (faction.getName() == Constants.UNION) {
				return new GroundUnit("AT Soldier", faction, 50, 10, Constants.SUPER_LIGHT,
						"The anti-tank infantery support",
						spawnPosition, 8, 100, Constants.HEAVY, 70, 100, true, 1);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new GroundUnit("Mechanised Infantery", faction, 50, 10, Constants.LIGHT,
			...
			if (faction.getName() == Constants.UNION) {
				return new GroundUnitWithMountedWeapon("MBT", faction, 500, 20, Constants.HEAVY,
						"Main battle tank of the Union",
						spawnPosition, 20, 100, Constants.HEAVY, 100, 100, true, 12,
						false, 30, 20, true);
			} else if (faction.getName() == Constants.FEDERATION) {
				return new GroundUnit("Lion", faction, 500, 30, Constants.HEAVY,
						"Heavy armored vehicle from the Federation",
						spawnPosition, 16, 120, Constants.SUPER_LIGHT, 120,
						100, true, 15);

			} else if (faction.getName() == Constants.REPUBLIC) {
				return new GroundUnit("Justice", faction, 400, 20, Constants.LIGHT,
